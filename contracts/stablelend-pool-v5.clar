...
;; ============================================
;; REAL-TIME INDEX CALCULATIONS (READ-ONLY)
;; ============================================

;; Calculate what the borrow index would be right now (without updating state)
(define-read-only (get-projected-borrow-index)
  (let (
    (blocks-elapsed (- block-height (var-get last-interest-update-block)))
    (current-borrow-index (var-get variable-borrow-index))
  )
    (if (is-eq blocks-elapsed u0)
      ...
      (let (
        (borrower-rate-per-block-numerator (* borrower-annual-rate-bps variable-borrow-index-precision))
        (rate-per-block-denominator (* u10000 blocks-per-year))
        (borrower-interest-multiplier (/ (* (* borrower-rate-per-block-numerator blocks-elapsed) current-borrow-index) 
                                          (* rate-per-block-denominator variable-borrow-index-precision)))
      )
        (+ current-borrow-index borrower-interest-multiplier)
      )
    )
  )
)

;; Calculate what the liquidity index would be right now (without updating state)
(define-read-only (get-projected-liquidity-index)
  (let (
    (blocks-elapsed (- block-height (var-get last-interest-update-block)))
    (current-liquidity-index (var-get liquidity-index))
    (utilization (get-utilization-rate))
    ...
    (if (is-eq blocks-elapsed u0)
      ...
      (let (
        ...
        (+ current-liquidity-index lender-interest-multiplier)
      )
    )
  )
)

;; Calculate real-time debt for a loan using projected borrow index
(define-private (calculate-realtime-loan-debt (principal uint) (user-borrow-index uint))
  (let (
    (projected-borrow-index (get-projected-borrow-index))
  )
    (if (is-eq user-borrow-index u0)
      principal
      (/ (* principal projected-borrow-index) user-borrow-index)
    )
  )
)

;; Calculate real-time lender balance using projected liquidity index
(define-private (calculate-realtime-lender-balance (shares uint) (user-index uint))
  (let (
    (projected-index (get-projected-liquidity-index))
    (share-value (get-share-value))
  )
    (if (is-eq user-index u0)
      (/ (* shares share-value) share-precision)
      (/ (* (* shares projected-index) share-value) 
         (* user-index share-precision))
    )
  )
)
...
;; Get lender's REAL-TIME balance and share information
  ...
  (let ((shares-entry (map-get? lender-shares { lender: lender })))
    (if (is-some shares-entry)
      (let (
        ...
        ;; Use REAL-TIME balance calculation with projected liquidity index
        (balance (calculate-realtime-lender-balance user-shares user-index))
      )
        (ok {
          shares: user-shares,
          balance: balance,
          ...
          user-liquidity-index: user-index
        })
      )
      (err ERR-NOT-AUTHORIZED)
    )
...
(define-read-only (get-protocol-stats)
  (ok {
    ...
    last-interest-update-block: (var-get last-interest-update-block),
...
;; Get complete loan details including REAL-TIME calculated debt
  ...
  (let ((loan-entry (map-get? loans { loan-id: loan-id })))
    (if (is-some loan-entry)
      (let (
        ...
        (principal-amount (get borrowed-amount loan-data))
        (user-borrow-index (get user-borrow-index loan-data))
        ;; Calculate REAL-TIME debt using projected borrow index
        (total-debt (calculate-realtime-loan-debt principal-amount user-borrow-index))
        ...
        (ok {
          ...
          current-borrow-index: (get-projected-borrow-index),
