    ...
    (let (
      ...
      (let ((remaining-shares (- user-total-shares shares-to-burn)))
        (if (is-eq remaining-shares u0)
          (begin
            (map-delete lender-shares { lender: tx-sender })
            (let ((lender-entry (map-get? unique-lenders { lender: tx-sender })))
              (if (is-some lender-entry)
                (if (get active (unwrap-panic lender-entry))
                  (begin
                    (map-set unique-lenders { lender: tx-sender } { active: false })
                    (var-set total-lenders (- (var-get total-lenders) u1))
                    true
                  )
                  true
      ...
      (try! (as-contract (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.usdcx
        transfer amount-to-withdraw tx-sender tx-sender none)))
...
(define-public (borrow (amount uint) (collateral-stx uint))
  (begin
    ...
    (let (
      ...
      (try! (add-loan-to-borrower tx-sender loan-id))
      
      (if (is-none (map-get? unique-borrowers { borrower: tx-sender }))
        (begin
          (map-set unique-borrowers { borrower: tx-sender } { active: true })
